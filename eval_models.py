...
def calcPagePrior(train_df, valid_df):
    ...
    instruments = sorted(list(set(labels)))
    instrument2idx = {c:i for i, c in enumerate(instruments)}
    
    # accumulate & normalize
    counts = np.zeros(len(instruments))
    for l in labels:
        counts[instrument2idx[l]] += 1
...
def calcAccuracy_fullpage(learner, path, train_df, valid_df, test_df, databunch = None, ensembled = False):
    ...
    instruments = sorted(set(labels))
    instrument2idx = {c:i for i, c in enumerate(instruments)}
    gt = torch.from_numpy(np.array([instrument2idx[l] for l in labels]))
